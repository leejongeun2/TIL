...
```py
Todo.objects.create(content="실습제출",prio
   ...: rity=5,deadline="2022-09-27")
```
...
```py
Todo.objects.order_by("-deadline").values()
```
...
```py
Todo.objects.filter(priority=5).order_by("
    ...: id")
```
...
```py
Todo.objects.filter(completed=True).order_
    ...: by("id")
```
